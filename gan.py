...
# 일반적인 GAN 의 형태 입니다.
# It is a common GAN form.
# 라벨을 구분하지 않습니다.
# Does not distinguish between labels.

# 제너레이터 (G)  The generator
def build_generator(z_prior):
    ...
    # 마지막은 활성화함수 tanh 로
    # Finally, the activation function tanh
    x_generate = tf.nn.tanh(h3)

    # generator save variable
    g_params = [w1, b1, w2, b2, w3, b3]

    return x_generate, g_params

# 디스크리미네이터 (D) Discriminator
def build_discriminator(x_data, x_generated, keep_prob):
    # 실제 이미지와 생성된 이미지를 합침
    # Combine the actual image with the generated image
    ...
    # # batch_size cut by y_data and y_generated respectively
    # ex)
    #   이미지 60000개, 배치 사이즈 256, 이미지 사이즈 28 * 28 이라면
    # 60000 images, a layout size of 256, and an image size of 28 * 28
    #   h3 shape : (257, 1)
    #   y_data shape : (256, 1)
...
def train():
    # mnist 로 학습할 경우 If you study with
    # mnist = input_data.read_data_sets('MNIST_data', one_hot=True)
    
    # phd08 로 학습할 경우 If you study with
    ...
    keep_prob = tf.placeholder(tf.float32, name="keep_prob") # dropout 퍼센트percent
    global_step = tf.Variable(0, name="global_step", trainable=False)

    # x_generated : generator 가 생성한 이미지, g_params : generater 의 TF 변수들
    # x_generated: image generated by generator, TF variables of g_params: generater
    x_generated, g_params = build_generator(z_prior)
    # 실제이미지, generater 가 생성한 이미지, dropout keep_prob 를 넣고 discriminator(경찰) 이 감별
    # Add a real image, an image generated by generater, dropout keep_prob, and discriminator
    y_data, y_generated, d_params = build_discriminator(x_data, x_generated, keep_prob)

    # loss 함수 ( D 와 G 를 따로 ) *
    # loss function (separate D and G) *
    ...
    # Learn to minimize each loss function with variables of discriminator and generator
...
# 학습 완료 후 테스트 (이미지로 저장)
# Test after completion (save as image)
def test():
